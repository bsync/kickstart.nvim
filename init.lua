...
vim.api.nvim_create_autocmd({ 'BufEnter', 'BufWinEnter' }, {
  group = vim.api.nvim_create_augroup('kickstart-python-group', { clear = true }),
  pattern = '*.py',
...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader>fr', builtin.lsp_references, { desc = '[F]ind LSP References' })
      vim.keymap.set('n', '<leader>fR', builtin.resume, { desc = '[F]ind [R]esume' })
      vim.keymap.set('n', '<leader>f.', builtin.oldfiles, { desc = '[F]ind Recent Files ("." for repeat)' })
      vim.keymap.set('n', '<leader>ft', ':TermSelect<CR>', { desc = '[F] an open Terminal' })
